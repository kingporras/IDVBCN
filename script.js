...
function getAuthUsers() {
  return (state.data?.players || []).map((player) => ({
    username: String(player.username || player.name || '').toLowerCase(),
    pin: String(player.pin ?? player.dorsal ?? ''),
    playerId: player.id,
...
function bindEvents() {
  $('loginForm').addEventListener('submit', (e) => {
    ...
    const found = getAuthUsers().find((u) => u.username === username && u.pin === pin);
    if (!found) {
...
async function init() {
  ...
  const stillExists = getAuthUsers().find((u) => u.username === session.username && u.pin === session.pin);
  if (!stillExists) {
    localStorage.removeItem('sessionUser');
    $('loginScreen').classList.remove('hidden');
    $('app').classList.add('hidden');
